...
Traccia: Devi modellare un sistema in cui diversi tipi di veicoli possono percorrere l’autostrada e pagare un pedaggio.
...
Gli attributi devono essere private e accessibili tramite getter e setter, con controlli sui valori (es. velocità ≥ 0).
...
public class Main {
    public static void main(String[] args) {
        ...
        ArrayList<Veicolo> listaVeicoli = new ArrayList<>();     //Creo un ArrayList che conterrà più tipologie di veicoli (grazie al polimorfismo)

        // Popolo l'ArrayList con tre tipologie diverse di veicoli derivanti dalla superclasse astratta "Veicolo":
        listaVeicoli.add(new Auto("AB123CD", 120, 2, 5));
        listaVeicoli.add(new Camion("EF456GH", 80, 4, 12000));
        listaVeicoli.add(new Moto("IJ789KL", 100, 2, 600));

        double totalePedaggi = 0;   //inizializzo il contatore del totale incassato in pedaggi autostradali a zero

        System.out.println("----------------------");
        // Foreach che cicla ogni singolo veicolo:
        for (Veicolo veicolo : listaVeicoli) {
            System.out.println(veicolo.getInfo());  //Eseguo il metodo corretto sovrascritto per ciascuna classe

            // Grazie al polimorfismo eseguo il metodo calcolaPedaggio() specifico per ogni classe:
            double pedaggio = veicolo.calcolaPedaggio();
            System.out.println("Pedaggio: euro " + pedaggio);

            totalePedaggi += pedaggio;
            System.out.println("----------------------");
        }
        
        System.out.println("Totale pedaggi incassati: euro " + totalePedaggi);
    }
}