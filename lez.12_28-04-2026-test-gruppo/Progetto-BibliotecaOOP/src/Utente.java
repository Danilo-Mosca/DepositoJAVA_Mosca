...
public class Utente {
    ...
    public void stampaPrestiti() {
        ...
        if (prestiti.isEmpty()) {
            System.out.println("Nessuna risorsa è stata presa in prestito!");
        }
        else {
        for (Risorsa risorsa : prestiti) {
            // risorsa.visualizzaDettagli();        // Non serve più: ho fatto l'override del metodo toString() in Risorsa
            System.out.println(risorsa);
            // Oppure:
            // System.out.println(risorsa.visualizzaDettagli());            // Sono equivalenti
            }
        }
    }
}
